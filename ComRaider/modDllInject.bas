...
Public Function InjectDLL(pid As Long, dllPath As String, remoteDllBase As Long, debugLog() As String) As Boolean
    ...
    Dim b() As Byte
    Dim buflen As Long
    Dim myKernel32Base As Long
    Dim myLoadLib As Long
    Dim deltaLoadLib As Long
    Dim buf() As Byte
    Dim hThread As Long
    ...
    If Not fso.FileExists(dllPath) Then
        push debugLog, "Could not find: " & dllPath
        Exit Function
    End If
    ...
    If hProcess < 1 Then
        push debugLog, "Could not open process"
        Exit Function
    End If
    
    push debugLog(), "Opening PID: " & pid & " Process Handle=" & hProcess
    ...
    myKernel32Base = GetModuleHandle("kernel32.dll")
    myLoadLib = GetProcAddress(myKernel32Base, "LoadLibraryA")
    
    If (myKernel32Base <> remoteDllBase) Then
        push debugLog, "Kernel32 was rebased in remote process"
        deltaLoadLib = myLoadLib - myKernel32Base
        lpfnLoadLib = remoteDllBase + deltaLoadLib
    Else
        lpfnLoadLib = myLoadLib
    End If
    
    push debugLog(), "Remote LoadLibraryA Addr = " & Hex(lpfnLoadLib)
    
    buflen = 10
    If ReadProcessMemory(hProcess, lpfnLoadLib, buf(0), buflen, 0) = 0 Then
        push debugLog, "Failed to read *lpfnLoadLib from hProcess??"
        CloseHandle hProcess
        Exit Function
    Else
        If buf(0) = 0 Then
            push debugLog, "*lpfnLoadLib=0??"
            CloseHandle hProcess
            Exit Function
        End If
    End If
