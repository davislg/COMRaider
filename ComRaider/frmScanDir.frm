...
Begin VB.Form frmScanDir 
   Caption         =   "Enumerate Safe for Scripting Controls"
   ClientHeight    =   5340
   ClientLeft      =   60
   ClientTop       =   345
   ClientWidth     =   10380
   ...
   Begin VB.Frame Frame1 
      ...
      Begin VB.TextBox txtGUID 
         ...
         Width           =   3975
      End
      Begin VB.CheckBox chkNoShowAudited 
         Caption         =   "hide audited classes"
         Height          =   255
         Left            =   8580
         TabIndex        =   2
         Top             =   60
...
Private Sub chkNoShowAudited_Click()
    If running Then Exit Sub
    If chkNoShowAudited.Visible Then 'so ignores form load setting
        ShowServersForPath basePath, False
    End If
End Sub
...
Function ShowServersForPath(basedir As String, Optional autoUnload As Boolean = True) As Boolean
    ...
    Me.caption = "Scanning " & UBound(tmp) & " CLSIDs found"
    DoEvents
    ...
    For Each v In tmp
         ...
         If InStr(1, path, basedir, vbTextCompare) < 1 Then GoTo nextOne
         ...
         If KeyExistsInCollection(killbitted, CStr(v)) Then
            li.ForeColor = vbRed
         End If
...
nextOne:
    ...
    If chkNoShowAudited.value = 1 Then
        Dim i As Long
    ...
    If lv.ListItems.Count > 0 Then
        ShowServersForPath = True
        Me.caption = lv.ListItems.Count & " classes found to objects registered in this path"
        Me.Visible = True
...
Private Sub Form_Load()
    ...
    'dont change ordering of these next two lines!
    chkNoShowAudited.value = GetSetting("ComRaider", "Settings", "chkNoShowAudited", 1)
    chkNoShowAudited.Visible = Options.DistributedMode
...
Private Sub Form_Unload(Cancel As Integer)
    SaveSetting "ComRaider", "Settings", "chkNoShowAudited", chkNoShowAudited.value
End Sub
